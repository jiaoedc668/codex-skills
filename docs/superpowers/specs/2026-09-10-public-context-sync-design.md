...
> 历史设计，对应 `boss-ip-v1.2.0`。2026-09-10 按 [实际实现](../../../boss-ip-video-script-writer/scripts/sync_public_context.py) 校正写入保证；当前使用入口见 [SKILL.md](../../../boss-ip-video-script-writer/SKILL.md)。

## 目标

为 `boss-ip-video-script-writer` 增加本地业务状态到 `chat-context/` 的公开安全同步器。业务项目中的人物、反馈台账、选题台账和共享偏好台账仍是唯一真相源；同步器只读取它们，准备全部公开快照后逐文件替换。
...
投影使用固定白名单，禁止递归保留未知字段。所有字符串字段经过敏感字段名和内部绝对路径检查；安全审查发现 `user_quote`、认证信息、内部路径、完整稿件键或疑似私人资料时失败。先把四个结果写入同一临时目录，全部生成后再逐文件替换目标。输入读取、安全校验或临时文件准备失败不会替换已有快照；替换阶段没有跨文件事务或回滚，中途 I/O 失败可能留下部分更新。输出目录由 CLI 指定，脚本不从仓库推断业务目录。

## 验收

测试覆盖：四类投影包含允许字段且不含敏感字段；输入业务文件字节不变；输入或安全校验失败时现有快照字节不变；四个输出均可解析且成功状态为 `current`；CLI 能使用显式路径运行。发布验证包括本目录全量 unittest、scripts compileall、四个 JSON 校验和 `git diff --check`。这些测试不证明替换阶段具有跨文件事务保证。
