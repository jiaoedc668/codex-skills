...
3. Python改动运行定向测试、全量unittest、compileall和git diff --check。规则改动用真实请求审读范围、事实、输出与反馈边界，不把词句匹配或机器通过当作脚本质量分。
   创作规则迭代还需用新的虚构 Raw Draft 做模型级回归：连读检查开场、情绪原话、说教与教程、真实阻力、批量推进差异、传播句、事件内生结尾和标题压缩。若仍批量同构或统一被说服，继续修规则再试写。仅记录机制与实际观察，不提交完整回归稿，不写入 schema、反馈台账或 direct_shoot；回归不是用户验收或传播效果证明。
