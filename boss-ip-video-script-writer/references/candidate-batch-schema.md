...
research_evidence必须原样带入选题池顶层的 `viral_expression_samples`，允许空数组或不足3条；填写的每条仍核验其真实性与原创边界。普通虚构evergreen不要求平台来源。新增可选布尔 `research_required`，热点、事实及明确研究任务须为true并提供非空sources或fact_sources；current_issue另需七天内heat_signals。省略标记保持旧v6可读，但不能被作者用于绕过事实核验，详见research-and-adaptation.md。topic_decision包含pool_id、topic_id、priority_category、concrete_event、concrete_anchor、audience_stake、strongest_counterargument、fage_choice、stakeholder_cost、ending_consequence、audience_response_space；逐字段匹配已选题。stakeholder_cost是当事人的现实损失，不强迫发哥承担代价。

persona_continuity继续引用人物模型有效版本；feedback_context必须等于独立台账当前投影，不自填评价。

反馈事件继续schema2，新增candidate_feedback承载“不错”等泛认可，复用preserve/avoid；用户原话与系统提炼来源分开，preserve投影为keep。它不参与可拍验收，详见feedback-loop.md。旧事件保持可读且不回填；含新事件的台账不能直接交给不认识该类型的旧版读取器。

## 蓝图与脚本
Raw Draft → Creative Review之后才填写以下结构。schema用来描述已经成立的故事，不允许schema反过来生成故事。Raw Draft与情绪触发句不新增程序对象、必填字段或评分，内部自由稿不进入公开输出。
