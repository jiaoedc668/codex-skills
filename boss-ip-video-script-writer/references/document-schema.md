...
- delivery为script_id和正整数version。剧情B、口播A，分别从01跨批次递增。
...
专项重写授权独立文件kind=rewrite_delivery_authorization，记录task_path/task_sha256、真实user_quote、scope和bindings。scope每项为candidate_id/script_id/old_source_path/old_source_sha256；bindings每项为script_id/source_sha256（完整envelope的规范JSON哈希）。本次原话“重写后直接交7个Word”只允许七篇旧源B01—B07剧情修订，不解锁其他稿。原任务、旧源、新源及修订关系都须校验，不能写假copy_confirmation或direct_shoot。

授权记录是可审计证据，并非外部密码签名。任务原文和实际会话授权是信任来源；检查者须逐条核对scope恰为此次七个旧稿，不能只相信文件自称获准。

## 编号与生成
正式交付/编号登记.json为事件数组，每项有稳定candidate_id、script_id、format、batch_id、version、source_sha256、word_sha256，交付事件另有路径和授权引用。旧七篇迁移V1作为编号基线，新版V2保留号；B01旧r2改r3，其余旧r1改r2。排版V3不是文案版本。
...
每份必须核对完整对白、标题、编号、版本、分镜和字幕；实际用本机Word只读打开并导出，渲染所有页面逐页看截断、空页、溢出、孤行和文字大小。内容哈希、文件存在或结构测试不能替代视觉检查。源与Word文件SHA256写入核验记录并对应登记；7份源、映射、逐篇改进依据及核验记录一并交付，旧版保留。
